...
<a class="btn btn-outline-primary" href="<?=e(url('supplier/batches.php'))?>">Manage batches</a>
</div>
<?php if ($errors):?>
...
<?php if ($edit):?>
<span class="channel-badge channel-<?=e(strtolower($form['listing_type']))?>">
<?=e($form['listing_type'] === 'B2B' ? 'B2B Wholesale' : 'B2C Retail')?>
</span>
<?php endif;?>
</div>
...
<small class="form-note d-block">The source batch and sales channel are permanent. Archive and recreate the listing to change either.</small>
<?php endif;?>
</div>
<div class="full batch-allocation" data-batch-summary hidden aria-live="polite">
<strong data-batch-summary-title>
</strong>
<span data-batch-summary-copy>
</span>
</div>
<div>
<label class="form-label" for="listed_quantity">Quantity allocated to this listing</label>
<div class="input-unit">
<input class="form-control" id="listed_quantity" type="number" step="0.01" min="0.01" name="listed_quantity" required value="<?=e($form['listed_quantity'])?>">
...
<div class="input-unit">
<input class="form-control" id="minimum_quantity" type="number" step="0.01" min="0.01" name="minimum_quantity" value="<?=e($form['minimum_quantity'])?>" <?=$form['listing_type'] === 'B2B' ? 'required' : 'disabled'?>>
<span data-batch-unit>units</span>
</div>
</div>
<div>
<label class="form-label" for="bulk_unit_price">Wholesale price per unit (BDT)</label>
<input class="form-control" id="bulk_unit_price" type="number" step="0.01" min="0.01" name="bulk_unit_price" value="<?=e($form['bulk_unit_price'])?>" <?=$form['listing_type'] === 'B2B' ? 'required' : 'disabled'?>>
</div>
</div>
</div>
...
<div class="input-unit">
<input class="form-control" id="bundle_size" type="number" step="0.01" min="0.01" name="bundle_size" value="<?=e($form['bundle_size'])?>" <?=$form['listing_type'] === 'B2C' ? 'required' : 'disabled'?>>
<span data-batch-unit>units</span>
</div>
</div>
<div>
<label class="form-label" for="fixed_unit_price">Retail price per unit (BDT)</label>
<input class="form-control" id="fixed_unit_price" type="number" step="0.01" min="0.01" name="fixed_unit_price" value="<?=e($form['fixed_unit_price'])?>" <?=$form['listing_type'] === 'B2C' ? 'required' : 'disabled'?>>
</div>
</div>
</div>
...
<span>A batch may be divided into separate wholesale and retail listings. Each listing ID belongs to only one buyer channel and has its own allocated quantity.</span>
</aside>
<section class="panel">
<div class="section-heading">
<div>
...
<a href="<?=e(url('supplier/listings.php'))?>">Clear</a>
</div>
</form>
...
<strong>Listing #<?=e($listing['listing_id'])?>
</strong>
<br>
<small class="muted">Batch #<?=e($listing['batch_id'])?>
</small>
</td>
<td data-label="Material">
<div class="material-cell">
<img class="textile-thumb" src="<?=e($listingImage['src'])?>" alt="<?=e($listingImage['alt'])?>" width="120" height="80" loading="lazy" decoding="async">
<span>
<strong>
<?=e($listing['material_type'])?>
</strong>
<small class="muted">
<?=e($listing['color'])?>
</small>
...
<span class="channel-badge channel-<?=e(strtolower($listing['listing_type']))?>">
<?=e($listing['listing_type'] === 'B2B' ? 'B2B Wholesale' : 'B2C Retail')?>
</span>
</td>
<td data-label="Allocation">
<?=e($listing['listed_quantity'])?>
