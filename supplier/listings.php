...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($action === 'archive' && $listingId) {
        ...
        $s->execute([$listingId,$supplierId]);
        setFlash($s->rowCount() ? 'success' : 'danger', $s->rowCount() ? 'Listing archived.' : 'Listing not found.');
    ...
    if (!$errors) {
        ...
        try {
            $s = $pdo->prepare('SELECT * FROM textile_batch WHERE batch_id=? AND supplier_id=? FOR UPDATE');
            $s->execute([$batchId,$supplierId]);
            $batch = $s->fetch();
            if (!$batch) {
                throw new RuntimeException('Batch not found.');
            }
            if ($listingId) {
                $s = $pdo->prepare("SELECT l.*,IF(bl.listing_id IS NULL,'B2C','B2B') listing_type FROM listing l JOIN textile_batch b ON b.batch_id=l.batch_id LEFT JOIN b2b_listing bl ON bl.listing_id=l.listing_id WHERE l.listing_id=? AND b.supplier_id=? FOR UPDATE");
                $s->execute([$listingId,$supplierId]);
                $old = $s->fetch();
                if (!$old) {
            ...
            $s = $pdo->prepare("SELECT COALESCE(SUM(listed_quantity),0) FROM listing WHERE batch_id=? AND status='Active' AND listing_id<>?");
            $s->execute([$batchId,$listingId ?: 0]);
            $other = (float)$s->fetchColumn();
...
$s = $pdo->prepare($listingSql);
$s->execute($listingParams);
$listings = $s->fetchAll();
