    ...
    <section class="panel mt-0">
        <div class="table-wrap">
            <table class="table align-middle responsive-table">
                <thead><tr><th>Order</th><th>Material</th><th>Supplier</th><th>Quantity</th><th>Total</th><th>Order</th><th>Payment</th><th>Action</th></tr></thead>
                <tbody>
                <?php foreach ($orders as $order): ?>
                    <tr>
                        <td data-label="Order"><strong>#<?= e($order['order_id']) ?></strong><br><small class="muted"><?= e(date('d M Y', strtotime($order['order_date']))) ?></small></td>
                        <td data-label="Material"><?= e($order['material_type']) ?><br><small class="muted"><?= e($order['color']) ?> · Listing #<?= e($order['listing_id']) ?></small></td>
                        ...
                        <td data-label="Actions">
                            <div class="action-row">
                                ...
                                <?php if (in_array($order['order_status'], ['Confirmed', 'Processing'], true) && (!$order['payment_status'] || $order['payment_status'] === 'Failed')): ?>
                                    <a class="btn btn-sm btn-primary" href="<?= e(url('payment.php?order_id=' . $order['order_id'])) ?>"><?= $order['payment_status'] === 'Failed' ? 'Retry payment' : 'Pay' ?></a>
                                <?php endif; ?>
                                <?php if (in_array($order['order_status'], ['Pending', 'Confirmed'], true)): ?>
                                    <form method="post" onsubmit="return confirm('Cancel this order and restore its reserved stock?')">
                                        <?= csrfField() ?>
                ...
                <?php endforeach; ?>
                <?php if (!$orders): ?><tr><td colspan="8" class="text-center muted py-4">You have not placed an order yet.</td></tr><?php endif; ?>
                </tbody>
            </table>
        </div>
    </section>
